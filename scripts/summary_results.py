...
def plot_heatmap(df):
    ...
    heatmap_data.index = [str(m).title() for m in heatmap_data.index]
...
def plot_scatter_word_limit(df):
    ...
    # Convert to numeric (in case of string type)
    plot_df["word_limit"] = pd.to_numeric(plot_df["word_limit"], errors="coerce")
    plot_df["weat"] = pd.to_numeric(plot_df["weat"], errors="coerce")
    plot_df = plot_df.dropna(subset=["word_limit", "weat"])
    # Only keep word_limit values 50, 100, 200
    plot_df = plot_df[plot_df["word_limit"].isin([50, 100, 200])]
    # Make word_limit categorical for y-axis
    plot_df["word_limit"] = plot_df["word_limit"].astype(int).astype(str)
